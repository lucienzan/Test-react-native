      ...
      transaction.map(item => {
        return (
          <View style={styles.transaction} key={item.id}>
            <MaterialIcons name="compare-arrows" size={32} color={item.type === "in" ? "green" : "brown"} />
            <View>
              <Text style={styles.text.payment}>{item.msg}</Text>
              <Text style={styles.text.muted}>{item.time}</Text>
          </View>
          </View>
        );
